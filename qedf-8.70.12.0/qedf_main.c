...
static int qedf_resume(struct pci_dev *pdev);
...
retry_probe:
	...
	qed_ops->common->set_dev_reuse(qedf->cdev, false);
...
static void __qedf_remove(struct pci_dev *pdev, int mode)
{
	...
	if (mode == QEDF_MODE_RECOVERY) {
		qed_ops->common->set_dev_reuse(qedf->cdev, true);
...
int qedf_resume(struct pci_dev *pdev)
{
	struct qedf_ctx *qedf;
	...
	QEDF_ERR(&qedf->dbg_ctx, "%s: driver does not support resume\n", __func__);

	return -EPERM;
}

int qedf_suspend(struct pci_dev *pdev, pm_message_t state)
{
	struct qedf_ctx *qedf;
	...
	QEDF_ERR(&qedf->dbg_ctx, "%s: driver does not support suspend\n", __func__);
